<x-app-layout>
    <div class="max-w-7xl mx-auto bg-white shadow-md rounded-lg p-6 my-2">
        <h2 class="text-2xl font-bold text-[#401457]">Edit Services</h2>
        <form action="{{ route('servicelist.update', $service->id) }}" method="POST" enctype="multipart/form-data"
            class="space-y-4">
            @csrf
            @method('PATCH')
            ...
            <div class="w-full grid grid-cols-1 md:grid-cols-3 gap-4">
                <div class="p-0 space-y-4">
                    ...
                    <div>
                        <label for="full_title_en" class="block text-sm font-medium text-[#401457]">Full Title</label>
                        <input value="{{ old('full_title.en', $service->full_title['en'] ?? '') }}" type="text"
                            name="full_title[en]" id="full_title_en"
                            class="mt-1 block w-full p-2 border rounded-md text-[#401457] text-sm">
                        <x-input-error class="mt-2" :messages="$errors->get('full_title.en')" />
                    </div>
                    <div>
                        <label for="content_display_en" class="block text-sm font-medium text-[#401457]">Content
                            Display</label>
                        <textarea name="content_display[en]" id="content_display_en" rows="4"
                            class="mt-1 block w-full p-2 border rounded-md text-[#401457] text-sm">{{ old('content_display.en', $service->content_display['en'] ?? '') }}</textarea>
                        <x-input-error class="mt-2" :messages="$errors->get('content_display.en')" />
                    </div>
                    <div>
                        ...
                        <textarea name="content[en]" id="content_en" rows="6"
                            class="mt-1 block w-full p-2 border rounded-md text-[#401457] text-[12px]">{{ old('content.en', $service->content['en'] ?? '') }}</textarea>
                ...
                <div class="p-0 space-y-4">
                    ...
                    <div>
                        <label for="full_title_kh" class="block text-sm font-medium text-[#401457]">Full Title</label>
                        <input value="{{ old('full_title.kh', $service->full_title['kh'] ?? '') }}" type="text"
                            name="full_title[kh]" id="full_title_kh"
                            class="mt-1 block w-full p-2 border rounded-md text-[#401457] text-sm">
                        <x-input-error class="mt-2" :messages="$errors->get('full_title.kh')" />
                    </div>
                    <div>
                        <label for="content_display_kh" class="block text-sm font-medium text-[#401457]">Content
                            Display</label>
                        <textarea name="content_display[kh]" id="content_display_kh" rows="4"
                            class="mt-1 block w-full p-2 border rounded-md text-[#401457] text-sm">{{ old('content_display.kh', $service->content_display['kh'] ?? '') }}</textarea>
                        <x-input-error class="mt-2" :messages="$errors->get('content_display.kh')" />
                    </div>
                    <div>
                        ...
                        <textarea name="content[kh]" id="content_kh" rows="6"
                            class="mt-1 block w-full p-2 border rounded-md text-[#401457] text-[12px]">{{ old('content.kh', $service->content['kh'] ?? '') }}</textarea>
                ...
                <div class="p-0 space-y-4">
                    ...
                    <div>
                        ...
                        <input value="{{ old('title.ch', $service->title['ch'] ?? '') }}" type="text"
                            name="title[ch]" id="title_ch"
                            class="mt-1 block w-full p-2 border rounded-md text-[#401457] text-sm">
                        <x-input-error class="mt-2" :messages="$errors->get('title.ch')" />
                    </div>
                    <div>
                        <label for="full_title_ch" class="block text-sm font-medium text-[#401457]">Full Title</label>
                        <input value="{{ old('full_title.ch', $service->full_title['ch'] ?? '') }}" type="text"
                            name="full_title[ch]" id="full_title_ch"
                            class="mt-1 block w-full p-2 border rounded-md text-[#401457] text-sm">
                        <x-input-error class="mt-2" :messages="$errors->get('full_title.ch')" />
                    </div>
                    <div>
                        <label for="content_display_ch" class="block text-sm font-medium text-[#401457]">Content
                            Display</label>
                        <textarea name="content_display[ch]" id="content_display_ch" rows="4"
                            class="mt-1 block w-full p-2 border rounded-md text-[#401457] text-sm">{{ old('content_display.ch', $service->content_display['ch'] ?? '') }}</textarea>
                        <x-input-error class="mt-2" :messages="$errors->get('content_display.ch')" />
                    </div>
                    <div>
                        ...
                        <textarea name="content[ch]" id="content_ch" rows="6"
                            class="mt-1 block w-full p-2 border rounded-md text-[#401457] text-[12px]">{{ old('content.ch', $service->content['ch'] ?? '') }}</textarea>
            ...
            <div class="w-full grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <label for="dropzone-icon{{ $service->id }}" id="drop-area-icon"
                        class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50">
                        <div id="icon-preview" style="background-image: url({{ asset($service->icon) }})"
                        ...
                        <input id="dropzone-icon{{ $service->id }}" type="file" class="hidden" name="icon"
                ...
                <div>
                    ...
                    <label for="dropzone-file{{ $service->id }}" id="drop-area"
                        class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-lg cursor-pointer bg-gray-50">
                        <div class="flex flex-col items-center justify-center pt-5 pb-6 w-full h-full bg-contain bg-center bg-no-repeat rounded-md text-center"
                            id="img-preview" style="background-image: url({{ asset($service->image) }})">
                            <p class="mb-2 text-[12px] sm:text-[14px] text-[#000]"><span class="font-semibold">Click
                                    to
                                    upload</span> or drag and drop</p>
                            <p class="text-xs text-[#000]">SVG, PNG, JPG or GIF (MAX. 5MB)</p>
                        </div>
                        <input id="dropzone-file{{ $service->id }}" type="file" class="hidden" name="image"
